...
func (tool *NodeInvokeTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	if err != nil {
		...
		view := nodeInvokeResult{
			...
			RecoveryAction: "Call nodes_status with this invocation_id before deciding whether to retry.",
		}
		return nodeInvocationError(
			"DISPATCH_UNCERTAIN",
...
func (runtime *nodeInvocationToolRuntime) prepare(
	...
	if binding.Executor != "" && binding.Executor != profile.Executor {
		return nodes.GatewayInvocationRecord{}, errors.New(
	...
	if err != nil {
		return nodes.GatewayInvocationRecord{}, errors.New("command input violates target policy")
	}
...
func boundedNodeInteger(args map[string]any, name string, fallback int, maximum int) (int, error) {
