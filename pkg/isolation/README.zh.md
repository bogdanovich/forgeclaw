...
`pkg/isolation` 为 `mintclaw` 启动的子进程提供进程级隔离能力。

它当前不会把 `mintclaw` 主进程自身放进沙箱中运行。
...
- 如果设置了 `MINTCLAW_HOME`，使用该值。
- 否则默认使用用户目录下的 `.mintclaw`。
...
隔离开启后，MintClaw 会打印生成后的隔离计划，便于排障。
