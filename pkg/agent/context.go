...
func (cb *ContextBuilder) getIdentity(includeToolUseRule bool) string {
	...
	if includeToolUseRule {
		rules = append(
			rules,
			fmt.Sprintf(
				"**Memory** - Prefer the memory tool when available. Use add for a new stable fact, replace for a correction, remove for explicit forgetting, and append_daily only for noteworthy transient events, decisions, progress, or unfinished context that may matter over the next few days. Do not record routine conversation. Otherwise update %s/memory/MEMORY.md or the appropriate daily note carefully.",
				workspacePath,
			),
		)
	}
