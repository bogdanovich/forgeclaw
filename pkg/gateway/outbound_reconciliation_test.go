...
func TestGatewayOutboundReconcilerPublishesCanonicalTextAndMedia(t *testing.T) {
	...
	reconciler, err := startGatewayOutboundReconciler(t.Context(), second, msgBus, admissions)
	if err != nil {
...
func TestGatewayOutboundReconcilerReleasesUnpublishedAdmission(t *testing.T) {
	...
	msgBus := bus.NewMessageBus()
	msgBus.Close()
	if _, err := startGatewayOutboundReconciler(t.Context(), second, msgBus, admissions); err == nil {
...
func TestGatewayOutboundReconcilerShutdownReleasesDelayedAdmission(t *testing.T) {
	...
	msgBus := bus.NewMessageBus()
	t.Cleanup(msgBus.Close)
	reconciler, err := startGatewayOutboundReconciler(context.Background(), second, msgBus, admissions)
	if err != nil {
		t.Fatalf("startGatewayOutboundReconciler() error = %v", err)
	}
	reconciler.stop()
