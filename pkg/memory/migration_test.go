...
func TestMigrateFromJSON_SkipsMetaJSONFiles(t *testing.T) {
	...
	if addErr := store.AddMessage(ctx, "agent:main:mintclaw:direct:mintclaw:test", "user", "keep me"); addErr != nil {
		t.Fatalf("AddMessage: %v", addErr)
	}
	if summaryErr := store.SetSummary(
		ctx,
		"agent:main:mintclaw:direct:mintclaw:test",
		"keep summary",
	); summaryErr != nil {
		t.Fatalf("SetSummary: %v", summaryErr)
	}

	metaPath := filepath.Join(sessionsDir, "agent_main_mintclaw_direct_mintclaw_test.meta.json")
	...
	history, err := store.GetHistory(ctx, "agent:main:mintclaw:direct:mintclaw:test")
	if err != nil {
		t.Fatalf("GetHistory: %v", err)
	}
	...
	summary, err := store.GetSummary(ctx, "agent:main:mintclaw:direct:mintclaw:test")
	if err != nil {
		t.Fatalf("GetSummary: %v", err)
	}
