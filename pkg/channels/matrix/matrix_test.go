...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("mintclaw")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@mintclaw hello", want: true},
		{text: "hi @mintclaw:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bmintclaw\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@mintclaw:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@mintclaw:matrix.org hello", want: "hello"},
		{in: "@mintclaw, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@mintclaw:matrix.org")},
				},
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body: "@mintclaw:matrix.org hello",
			},
			want: true,
		},
		{
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@mintclaw:matrix.org">MintClaw</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40mintclaw%3Amatrix.org">MintClaw</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@mintclaw:matrix.test"), "")
	if err != nil {
