...
func (session *peer) request(
	...
	response, _, err := session.requestWithDispatchCommit(
		ctx,
		method,
		params,
		idempotencyKey,
...
// requestWithDispatchCommit runs commit after local request admission but
// immediately before the first write. A successful commit means a write may
// have occurred even when the eventual response is unavailable.
func (session *peer) requestWithDispatchCommit(
	ctx context.Context,
	method string,
	params json.RawMessage,
	idempotencyKey string,
	...
	session.pendingMu.Unlock()
	dispatched, err := session.writeEnvelopeWithDispatchCommit(ctx, protocol.Envelope{
...
func (session *peer) writeEnvelope(ctx context.Context, envelope protocol.Envelope) error {
	_, err := session.writeEnvelopeWithDispatchCommit(ctx, envelope, nil)
	return err
}

func (session *peer) writeEnvelopeWithDispatchCommit(
	ctx context.Context,
	envelope protocol.Envelope,
	commit func() error,
) (bool, error) {
