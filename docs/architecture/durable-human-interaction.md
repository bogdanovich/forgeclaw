...
It is not model-authored. Runtime always adds the exact runtime-owned tool name
and never generically serializes tool arguments into the human prompt. If a tool
later needs richer presentation, it should expose a dedicated trusted approval
descriptor rather than rely on a universal argument renderer.
