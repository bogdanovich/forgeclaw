...
In MintClaw, routing has two user-facing parts:
...
If no rule matches, MintClaw falls back to the default agent.
...
- MintClaw resolves the light model at startup; if it is invalid, routing is disabled
