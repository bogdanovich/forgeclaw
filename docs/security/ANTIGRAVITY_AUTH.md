...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in MintClaw.
...
10. [Creating a New Provider in MintClaw](#creating-a-new-provider-in-mintclaw)
...
const antigravityPlugin = {
  ...
  register(api: MintClawPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<MintClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.26
- MintClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.mintclaw/auth.json`:
...
mintclaw auth login --provider antigravity
...
mintclaw agent -m "Hello" --model antigravity/gemini-3-flash
```
...
export MINTCLAW_AGENTS_DEFAULTS_MODEL=antigravity/gemini-3-flash

# Override model_list with an Antigravity entry
export MINTCLAW_MODEL_LIST='[{"model_name":"ag-flash","provider":"antigravity","model":"gemini-3-flash"}]'
```
...
- **Source Files:**
  ...
  - `pkg/auth/store.go` - Auth credential storage (`~/.mintclaw/auth.json`)
  - `pkg/providers/factory.go` - Provider factory and protocol routing
  - `pkg/providers/types.go` - Provider interface definitions
  - `cmd/mintclaw/internal/auth/helpers.go` - Auth CLI commands
...
### "Token expired"
- Refresh OAuth tokens: `mintclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.mintclaw/auth.json`
- Re-run `mintclaw auth login --provider antigravity`
