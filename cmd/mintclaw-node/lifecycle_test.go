...
func TestSystemInstallRequiresExplicitConfigurationAndUser(t *testing.T) {
	...
	got, err := resolveLifecycleConfigPath("/etc/mintclaw/node.json", true, true)
	if err != nil || got != "/etc/mintclaw/node.json" {
...
func TestResolveServiceAccountRequiresUnprivilegedUser(t *testing.T) {
	...
	got, err := resolveServiceAccount("mintclaw", func(string) (*user.User, error) {
		return &user.User{Uid: "1001", Gid: "1001", Username: "mintclaw"}, nil
	}, groups)
	if err != nil || got != "mintclaw" {
...
func TestWriteLifecycleStatusJSON(t *testing.T) {
	want := lifecycleStatus{
		...
		Manager:   "systemd",
		Scope:     "user",
		Service:   "mintclaw-node-main.service",
		UnitPath:  "/tmp/mintclaw-node-main.service",
		Installed: true,
		Active:    true,
		State:     "active",
	}
