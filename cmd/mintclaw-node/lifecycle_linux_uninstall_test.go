...
func TestSystemdLifecycleUninstallCompensatesFailedDisable(t *testing.T) {
	...
	lifecycle := &systemdLifecycle{
		...
		run: func(_ context.Context, _ bool, args ...string) (systemdRunResult, error) {
			switch {
			case reflect.DeepEqual(args, systemdUnitShowArgs("mintclaw-node-main.service")):
				return enabledSystemdUnitResult(unitPath, "active"), nil
			case reflect.DeepEqual(args, []string{"disable", "--now", "mintclaw-node-main.service"}):
				enabled = false
				active = false
				return systemdRunResult{Output: "partial disable failed", ExitCode: 1}, nil
			case reflect.DeepEqual(args, []string{"enable", "mintclaw-node-main.service"}):
				enabled = true
			case reflect.DeepEqual(args, []string{"start", "mintclaw-node-main.service"}):
				active = true
			default:
				t.Fatalf("unexpected systemctl call: %v", args)
			}
