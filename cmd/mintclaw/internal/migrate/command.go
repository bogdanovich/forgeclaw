...
import (
	...
	"github.com/bogdanovich/mintclaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to mintclaw",
		Args:  cobra.NoArgs,
		Example: `  mintclaw migrate
  mintclaw migrate --from openclaw
  mintclaw migrate --dry-run
  mintclaw migrate --refresh
  mintclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.mintclaw)")
