...
func printOnboardPlain(logo string, encrypt bool, configPath string) {
	...
	fmt.Println("")
	if encrypt {
		fmt.Println("  3. Chat: mintclaw agent -m \"Hello!\"")
	} else {
		fmt.Println("  2. Chat: mintclaw agent -m \"Hello!\"")
	}
}
...
func printOnboardFancy(logo string, encrypt bool, configPath string) {
	...
	ready := titleBarStyle().Render(logo+" mintclaw is ready!") + "\n"
	fmt.Println()
...
func buildOnboardingSteps(encrypt bool, configPath string) string {
	...
	if encrypt {
		b.WriteString("1. Set your encryption passphrase before starting mintclaw:\n")
		b.WriteString("   export MINTCLAW_KEY_PASSPHRASE=<your-passphrase>   # Linux/macOS\n")
		b.WriteString("   set MINTCLAW_KEY_PASSPHRASE=<your-passphrase>      # Windows cmd\n\n")
